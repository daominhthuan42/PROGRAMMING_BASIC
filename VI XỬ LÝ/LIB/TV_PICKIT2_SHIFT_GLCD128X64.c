...
{  
  INt16 TAM;
  if(GLCD_COLOR)  TAM=0xffff; else TAM=0;
  for (DOC=0; DOC < GLCD_DOC; DOC++) 
  { 
    for (NGANG=0; NGANG < GLCD_NGANG; NGANG++) 
    { 
      gdram_vdk.PIXEL[DOC][NGANG].word = TAM; 
    } 
  } 
...
{ 
  ...
  if (color == 1) bit_set (gdram_vdk.PIXEL[DOC][NGANG].word,TAM); 
  else bit_clear (gdram_vdk.PIXEL[DOC][NGANG].word, TAM); 
  gdram_vdk.refresh = TRUE; 
} 
